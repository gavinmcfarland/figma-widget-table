<script>
	...
	async function onLoad(event) {
		...
		if (message.dataEndpoint?.api === "googleSheets") {
			...
			googleSheetsInput.value = message.dataEndpoint.url
				? message.dataEndpoint.url
				: null;
			if (message.dataEndpoint.sheetName) {
				googleSheetsSheetNameInput.value =
					message.dataEndpoint.sheetName;
			}
		...
		googleSheetsButton.addEventListener("click", () => {
			...
			if (googleSheetsInput.value && url) {
				gsheetsAPI({
						...
						parent.postMessage(
							{
								pluginMessage: {
									...
									data: result.values,
									api: "googleSheets",
									url: googleSheetsInput.value,
									sheetName: googleSheetsSheetNameInput.value,
								},
							},
							"*",
						);
					...
					.catch(() => {
						parent.postMessage(
							{ pluginMessage: { type: "api-error" } },
							"*",
						);
					});
			} else {
				parent.postMessage(
					{ pluginMessage: { type: "invalid-link" } },
					"*",
				);
			}
		});
...
<div class="m-xsmall type--small">
	...
	<div class="input" style="display: flex; gap: 16px">
		...
		<div style="width: 80px; flex-grow: 1">
			<label
				...
				<input
					style="margin-top: 8px"
					id="googleSheetsSheetNameInput"
					bind:this={googleSheetsSheetNameInput}
					type="input"
					class="input__field"
	...
	<div style="display: flex; gap: 16px">
		<button
			...
			id="googleSheetsButton"
			bind:this={googleSheetsButton}>Import</button
		>
		<button
			class="button button--secondary mt-xsmall"
			id="detachApiButton"
			bind:this={detachApiButton}>Unlink</button
		>
	</div>
	...
	<button
		class="button button--primary mt-xsmall"
		id="button"
		bind:this={button}>Import</button
	>
</div>
