	...
	onMount(async () => {
			...
			parent.postMessage(
				{
					pluginMessage: {
						type: "settings-saved",
						settings: {
							navigateOnEnter: navigateOnEnterInput.checked,
						},
					},
				},
				"*",
		...
		firstRowAsHeader.addEventListener("input", () => {
			parent.postMessage(
				{
					pluginMessage: {
						type: "widget-first-row-as-header-saved",
						firstRowAsHeader: firstRowAsHeader.checked,
					},
				},
				"*",
			);
		...
		useDarkTheme.addEventListener("input", () => {
			...
			parent.postMessage(
				{ pluginMessage: { type: "widget-theme-saved", theme } },
				"*",
			);
		});
...
<div class="m-xsmall type--small">
	...
	<div class="checkbox">
		<input
			id="navigateOnEnterInput"
			bind:this={navigateOnEnterInput}
			type="checkbox"
			class="checkbox__box"
		/>
		<label for="navigateOnEnterInput" class="checkbox__label"
			>Navigate to cell below on enter</label
		>
	</div>
	<div class="checkbox">
		<input
			id="useDarkTheme"
			bind:this={useDarkTheme}
			type="checkbox"
			class="checkbox__box"
		/>
		<label for="useDarkTheme" class="checkbox__label"
			>Use dark theme for widget</label
		>
	</div>
	<div class="checkbox">
		<input
			id="firstRowAsHeader"
			bind:this={firstRowAsHeader}
			type="checkbox"
			class="checkbox__box"
		/>
		<label for="firstRowAsHeader" class="checkbox__label"
			>Use first row as header</label
		>
	</div>
	<hr />
	<p class="type--bold">Export data</p>
	<button
		class="button button--primary mt-xsmall"
		id="exportFile"
		bind:this={exportFile}>Export as CSV</button
	>
	<br />
	<hr />
	<p class="type--bold">Clear table</p>
	<button
		class="button button--primary-destructive mt-xsmall"
		id="clearTable"
		bind:this={clearTable}>Clear Table</button
	>
</div>
